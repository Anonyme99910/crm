  ...
  <div class="p-6">
    <div class="flex justify-between items-center mb-6">
      ...
      <div class="text-sm text-gray-600">
        <span class="font-semibold">{{ unreadCount }}</span> unread messages
      </div>
    </div>
    ...
    <div v-else-if="submissions.length === 0" class="text-center py-12 bg-white rounded-lg shadow">
      ...
      <p class="mt-4 text-gray-500">No contact submissions yet</p>
    </div>
    ...
    <div v-else class="space-y-4">
      <div v-for="submission in submissions" :key="submission.id" 
           ...
           :class="{ 'border-l-4 border-bronze': !submission.is_read }">
        <div class="p-6">
          <div class="flex justify-between items-start mb-4">
            <div class="flex-1">
              <div class="flex items-center gap-3 mb-2">
                <h3 class="text-lg font-bold text-gray-900">{{ submission.name }}</h3>
                <span v-if="!submission.is_read" class="px-2 py-1 bg-bronze text-white text-xs rounded-full">New</span>
              </div>
              <div class="text-sm text-gray-600 space-y-1">
                <p><strong>Email:</strong> <a :href="`mailto:${submission.email}`" class="text-bronze hover:underline">{{ submission.email }}</a></p>
                <p v-if="submission.phone"><strong>Phone:</strong> {{ submission.phone }}</p>
                <p v-if="submission.subject"><strong>Subject:</strong> {{ submission.subject }}</p>
...
const markAsRead = async (id) => {
  try {
    await axios.put(`api/contact-submissions/${id}/read`);
