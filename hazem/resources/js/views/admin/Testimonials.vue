          ...
          <div class="bg-gray-50 p-4 rounded-lg">
            ...
            <div class="flex items-center gap-4">
              <div v-if="editing.client_image" class="relative">
                <img :src="`/hazem/storage/${editing.client_image}`" class="w-24 h-24 rounded-full object-cover border-4 border-bronze shadow-lg">
...
const saveTestimonial = async () => {
  ...
  try {
    if (editing.value.id) {
      await axios.put(`api/testimonials/${editing.value.id}`, editing.value);
      toast.success('Testimonial updated');
    } else {
      await axios.post('api/testimonials', editing.value);
...
const deleteTestimonial = async (testimonial) => {
  ...
  try {
    await axios.delete(`api/testimonials/${testimonial.id}`);
